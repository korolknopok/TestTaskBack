...
public class PostService
{
        ...
        PaginationModel pagination)
    {
        ...
        IQueryable<Post> query = _blogDbContext.Posts
            .AsNoTracking()
        ...
        {
            var existingTagsCount = await _blogDbContext.Tags
                .Where(t => filter.TagIds.Contains(t.Id))
                .CountAsync();
        ...
        {
            var userCommunityIds = await _blogDbContext.Communities
                .Where(c => c.Creator.Id == userId
                    || c.Administrators.Any(a => a.Id == userId)
                    || c.Subscribers.Any(s => s.Id == userId))
                .Select(c => c.Id)
                .ToListAsync();
            query = query.Where(p => p.CommunityId.HasValue && userCommunityIds.Contains(p.CommunityId.Value));
        }
        ...
        var posts = await query
            .Select(p => new PostDto
            {
                ...
                HasLike = p.LikedBy.Any(lb => lb.Id == userId),
